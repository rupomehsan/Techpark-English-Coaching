...
class GetAllData
{
    ...
    public static function execute()
    {
        try {
            ...
            $end_date = request()->input('end_date');
                            ...
                            $with = ['student_id', 'batch_id', 'live_course_id'];
    ...
    $q->where('student_id', 'like', '%' . $searchKey . '%');    

    $q->orWhere('batch_id', 'like', '%' . $searchKey . '%');    

    $q->orWhere('live_course_id', 'like', '%' . $searchKey . '%');    

    $q->orWhere('student_info', 'like', '%' . $searchKey . '%');    

    $q->orWhere('enrolled_at', 'like', '%' . $searchKey . '%');    

    $q->orWhere('payment_status', 'like', '%' . $searchKey . '%');    

    $q->orWhere('amount_paid', 'like', '%' . $searchKey . '%');    

    $q->orWhere('transaction_id', 'like', '%' . $searchKey . '%');    

    $q->orWhere('amount', 'like', '%' . $searchKey . '%');    

    $q->orWhere('payment_details', 'like', '%' . $searchKey . '%');    

    $q->orWhere('method', 'like', '%' . $searchKey . '%');              
