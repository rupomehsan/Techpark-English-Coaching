        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    ...
                    {{ isEditMode ? 'Edit Why Learn Item' : 'Create New Why Learn Item' }}
                </h5>
            </div>
            <div class="card-body">
                <form @submit.prevent="submitForm">
                    ...
                    <div class="form-group">
                        ...
                        <input 
                            ...
                            placeholder="Enter why learn reason"
                            required
                        >
...
export default {
    ...
    computed: {
        ...
        isEditMode() {
            return this.$route.name === 'CourseWhyLearnEdit' && this.$route.params.slug;
        },
    ...
    methods: {
        ...
        async loadItemData() {
            ...
            try {
                ...
                console.log('Loading why learn item data for slug:', itemSlug);
                ...
                if (response.data && response.data.status === 'success') {
                    ...
                    window.s_error('Failed to load why learn item data');
                }
            } catch (error) {
                console.error('Error loading why learn item data:', error);
                window.s_error('Failed to load why learn item data');
            } finally {
                this.loading = false;
            }
        ...
        async submitForm() {
            ...
            try {
                ...
                console.log('Submitting why learn item data:', itemData);
                
                let response;
                if (this.isEditMode) {
                    const itemSlug = this.itemSlug;
                    response = await axios.post(`course-why-you-learn-from-uses/update/${itemSlug}`, itemData);
                } else {
                    response = await axios.post('course-why-you-learn-from-uses/store', itemData);
                }
                
                console.log('Form - Submit Response:', response.data);
                
                if (response.data && response.data.status === 'success') {
                    const message = this.isEditMode ? 'Why learn item updated successfully!' : 'Why learn item created successfully!';
        ...
        goBack() {
            this.$router.push({ name: 'CourseWhyLearn' });
        },
    ...
    async mounted() {
        console.log('CourseWhyLearnForm mounted');
