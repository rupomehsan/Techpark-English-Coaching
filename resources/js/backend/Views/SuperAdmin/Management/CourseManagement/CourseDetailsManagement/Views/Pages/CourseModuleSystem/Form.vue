        ...
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    ...
                    {{ isEditMode ? 'Edit Course Module' : 'Create New Course Module' }}
                </h5>
            </div>
            <div class="card-body">
                <form @submit.prevent="submitForm">
                    ...
                    <!-- Serial -->
                    <div class="form-group">
                        <label for="serial" class="form-label">Serial Order</label>
                        <input 
                            type="number" 
                            id="serial"
                        ...
                        <small class="form-text text-muted">
                            This determines the order in which modules are displayed.
                        </small>
                    </div>
                    ...
                    <div class="form-actions">
                        <div class="d-flex justify-content-between">
                            ...
                            >
                                ...
                                {{ submitting ? 'Processing...' : (isEditMode ? 'Update Module' : 'Create Module') }}
                            </button>
                        </div>
                    </div>
                </form>
...
export default {
    ...
    computed: {
        ...
        isEditMode() {
            return this.$route.name === 'CourseModuleEdit' && this.$route.params.slug;
        },
    ...
    methods: {
        ...
        async loadItemData() {
            ...
            try {
                ...
                console.log('Loading course module data for slug:', itemSlug);
                
                const response = await axios.get(`course-modules/${itemSlug}`);
                ...
                if (response.data && response.data.status === 'success') {
                    ...
                    window.s_error('Failed to load course module data');
                }
            } catch (error) {
                console.error('Error loading course module data:', error);
                window.s_error('Failed to load course module data');
            } finally {
                this.loading = false;
            }
        ...
        validateForm() {
            ...
            if (!this.formData.title?.trim()) {
                this.errors.title = 'Module title is required';
            }
            
            if (this.formData.serial && this.formData.serial < 1) {
                this.errors.serial = 'Serial must be a positive number';
            }
        ...
        async submitForm() {
            ...
            try {
                ...
                console.log('Submitting course module data:', itemData);
                
                let response;
                if (this.isEditMode) {
                    const itemSlug = this.itemSlug;
                    response = await axios.post(`course-modules/update/${itemSlug}`, itemData);
                } else {
                    response = await axios.post('course-modules/store', itemData);
                }
                
                console.log('Form - Submit Response:', response.data);
                
                if (response.data && response.data.status === 'success') {
                    const message = this.isEditMode ? 'Course module updated successfully!' : 'Course module created successfully!';
                    window.s_alert(message);
                    this.goBack();
                } else {
                    window.s_error('Something went wrong while saving the module.');
                }
        ...
        goBack() {
            this.$router.push({ name: 'CourseModule' });
        },
    ...
    async mounted() {
        console.log('CourseModuleForm mounted');
