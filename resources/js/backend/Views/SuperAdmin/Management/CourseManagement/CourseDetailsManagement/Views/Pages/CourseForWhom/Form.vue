<template>
    <div class="course-for-whom-form">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title mb-0">
                    <i :class="isEditMode ? 'fas fa-edit mr-2' : 'fas fa-plus mr-2'"></i>
                    {{ isEditMode ? 'Edit Target Audience Item' : 'Create New Target Audience Item' }}
                </h5>
            </div>
            <div class="card-body">
...
export default {
    ...
    computed: {
        ...
        isEditMode() {
            return this.$route.name === 'CourseForWhomEdit' && this.$route.params.slug;
        },
    ...
    methods: {
        ...
        async loadItemData() {
            ...
            try {
                ...
                console.log('Loading for whom item data for slug:', itemSlug);
                ...
                if (response.data && response.data.status === 'success') {
                    ...
                    window.s_error('Failed to load target audience item data');
                }
            } catch (error) {
                console.error('Error loading target audience item data:', error);
                window.s_error('Failed to load target audience item data');
            } finally {
                this.loading = false;
            }
        ...
        async submitForm() {
            ...
            try {
                ...
                console.log('Submitting target audience item data:', itemData);
                
                let response;
                if (this.isEditMode) {
                    const itemSlug = this.itemSlug;
                    response = await axios.post(`course-for-whoms/update/${itemSlug}`, itemData);
                } else {
                    response = await axios.post('course-for-whoms/store', itemData);
                }
                
                console.log('Form - Submit Response:', response.data);
                
                if (response.data && response.data.status === 'success') {
                    const message = this.isEditMode ? 'Target audience item updated successfully!' : 'Target audience item created successfully!';
        ...
        goBack() {
            this.$router.push({ name: 'CourseForWhom' });
        },
    ...
    async mounted() {
        console.log('CourseForWhomForm mounted');
