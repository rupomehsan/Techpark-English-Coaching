...
export default {
    ...
    methods: {
                ...
                if ([200, 201].includes(response.statusCode)) {
                    ...
                    setTimeout(() => {
                        ...
                        if (response && response.data && response.data.slug) {
                            console.log(
                                "Redirecting to course details with slug:",
                                response.data.slug,
                            );
                            this.$router.push({
                                name: "CourseDetails",
        ...
        resetForm() {
            ...
            if (
                $("#what_is_this_course").length &&
                $("#what_is_this_course").summernote
            ) {
                $("#what_is_this_course").summernote("code", "");
            }
            if (
                $("#why_is_this_course").length &&
                $("#why_is_this_course").summernote
            ) {
        ...
        async initializeCKEditor() {
            try {
                if (window.ClassicEditor) {
                    ...
                    this.editor = await window.ClassicEditor.create(
                        this.$refs.editor,
                        {
        ...
        setSummernoteContent(courseData) {
            ...
            setTimeout(() => {
                if (
                    courseData.what_is_this_course &&
                    $("#what_is_this_course").length &&
                    $("#what_is_this_course").summernote
                ) {
                    $("#what_is_this_course").summernote(
                    ...
                    if (response.data.course_category_id) {
                        this.store.setFormData({
                            course_category_id: Number(
                                response.data.course_category_id,
                            ),
                        });
                    }
                    // Set image preview for existing course
